...
public class Tests {
    ...
    public void test_9() {
        ...
        //сначала убираем нулы, чтоб не мешали методу figure
        List<WebElement> noNullText = elements.stream().filter(el -> el.getText() != null).toList();
        //фильтруем и сортируем элементы с текстом
        List<WebElement> elementsText = noNullText.stream()
                .filter(el -> parse(el.getText()) >= 500)
                .sorted((a, b) -> {
                    int x = 0;
                    if (parse(a.getText()) > parse(b.getText())) {
                        x = 1;
                    } else {
                        x = -1;
                    }
                ...
                .toList();
        //elementsText.stream().forEach(el -> System.out.println(el.getText() + " " + el.getValue()));


        //сначала убираем нулы, чтоб не мешали методу figure
        List<WebElement> noNullValue = elements.stream().filter(el -> el.getValue() != null).toList();
        //фильтруем и сортируем элементы со значением
        List<WebElement> elementsVal = noNullValue.stream()
                .filter(el -> parse(el.getValue()) >= 500)
                .sorted((a, b) -> {
                    int x = 0;
                    if (parse(a.getValue()) > parse(b.getValue())) {
                        x = -1;
                    } else {
                        x = 1;
                    }
                ...
                .toList();
        //elementsVal.stream().forEach(el -> System.out.println(el.getText() + " " + el.getValue()));

        //Объединим листы
        List<WebElement> newList = new ArrayList<>();
        newList.addAll(elementsText);
        newList.addAll(elementsVal);
        newList.stream().forEach(el -> System.out.println(el.getText() + " " + el.getValue()));

    }


    public int parse(String text) {
        String stringText = text.replaceAll("\\D+", "");
        int figure = Integer.parseInt(stringText);
        return figure;
    }
